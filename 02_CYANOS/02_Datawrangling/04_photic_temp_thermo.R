...
if (!dir.exists("02_CYANOS/Figs/Daily_interp_Casts")) {
  dir.create("02_CYANOS/Figs/Daily_interp_Casts", recursive = TRUE)
}
...
#will save figures to these files
if (!dir.exists("02_CYANOS/Figs/Thermocline")) {
  dir.create("02_CYANOS/Figs/Thermocline", recursive = TRUE)
}
...
write.csv(photic_zone_frame, "02_CYANOS/CSVs/photic_zone_frame.csv", row.names = FALSE)
...
#write.csv(temp_depths_cleaned, "02_CYANOS/CSVs/temp_depths_cleaned.csv")
...
for (yr in years) {
  ...
  # Save plot
  ggsave(filename = paste0("02_CYANOS/Figs/Daily_interp_Casts/", yr, "_raw_casts.png"),
         plot = plot_casts,
         width = 12,
         height = 10,
         dpi = 300)
...
for (yr in years) {
  ...
  # Save plot
  ggsave(filename = paste0("02_CYANOS/Figs/Thermocline/", yr, "_raw_casts.png"),
         plot = plot_casts,
         width = 12,
         height = 10,
         dpi = 300)
...
write.csv(final_photic_thermo, "02_CYANOS/CSVs/final_photic_thermo.csv", row.names = FALSE)
