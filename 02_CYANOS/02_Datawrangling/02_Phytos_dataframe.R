...
write.csv(phytos, "02_CYANOS/CSVs/phytos.csv", row.names = FALSE)
phytos <- read.csv("02_CYANOS/CSVs/phytos.csv")
...
phytosplot <- ggplot(plot_dat, aes(x = DayOfYear, y = as.factor(Year), group = Year)) +
  ...
  labs(
    x = "Day of Year",
    y = "Year",
    title = "Fluoroprobe Data Availability 2015-2024 - Bluegreens"
  ) +
  scale_x_continuous(
...
dir.create("02_CYANOS/Figs/Data_availability", recursive = TRUE, showWarnings = FALSE)

ggsave(
  "02_CYANOS/Figs/Data_availability/TotalPhytos2025pub.png",
  phytosplot,
  width = 8,
  height = 6,
  dpi = 400
...
if (!dir.exists("02_CYANOS/Figs")) {
  dir.create("02_CYANOS/Figs")
}

if (!dir.exists("02_CYANOS/Figs/Phytos_viz")) {
  dir.create("02_CYANOS/Figs/Phytos_viz")
}
...
for (type in types) {
  ...
  for (yr in years) {
    ...
    dir.create("02_CYANOS/Figs/raw_flora_casts", recursive = TRUE, showWarnings = FALSE)
    
    # Save plot
    ggsave(
      filename = paste0("02_CYANOS/Figs/raw_flora_casts/", type, "_", yr, "_raw_casts.png"),
      plot = plot_casts,
      width = 12,
      height = 10,
      dpi = 300
...
for (var in pigment_vars) {
  ...
  for (yr in years) {
    ...
    ggsave(
      filename = paste0("02_CYANOS/Figs/raw_flora_casts/", var, "_", yr, "_raw_casts.png"),
      plot = plot_casts,
      width = 12,
      height = 10,
      dpi = 300
...
depth_plot <- ggplot(boxplot_Data, aes(x = factor(Year), y = DCM_depth)) +
  ...
  ggtitle("A   Deep Chlorophyll Maximum (DCM) Depth - Bluegreens") +
...
mag_plot <- ggplot(boxplot_Data, aes(x = factor(Year), y = max_conc)) +
  ...
  ggtitle("B   Deep Chlorophyll Maximum (DCM) Peak Magnitude - Bluegreens") +
  scale_y_continuous(
...
ggsave(
  ...
  plot = panel_plot,
  path = "02_CYANOS/Figs/Phytos_viz",
...
write.csv(frame_weeks, "02_CYANOS/CSVs/frame_weeks.csv", row.names = FALSE)
write.csv(final_phytos, "02_CYANOS/CSVs/final_phytos.csv", row.names = FALSE)
...
sig_both <- p_depth / p_mag_nolegend +
  ...
  plot_annotation(
    title = "Year-wise Pairwise Significance (BH-adjusted) - Bluegreens",
    theme = theme(plot.title = element_text(face = "bold", hjust = 0.5))
  )

ggsave("02_CYANOS/Figs/Phytos_viz/kruskal-wallis.png",
...
d <- ggplot(summary_df, aes(x = factor(Year))) +
  ...
  labs(
    y = "DCM depth (m)",
    x = "Year",
    title = "A   DCM Depth Median and Mean ± SE by Year - Bluegreens"
  ) +
...
p <- ggplot(summary_df, aes(x = factor(Year))) +
  ...
  labs(
    ...
    x = "Year",
    title = "B   DCM Magnitude Median and Mean ± SE by Year - Bluegreens"
  ) +
  theme_classic(base_size = 13) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggsave("02_CYANOS/Figs/Phytos_viz/phytoplankton_summary.png", plot = combined_plot, width = 8, height = 10, dpi = 600)
