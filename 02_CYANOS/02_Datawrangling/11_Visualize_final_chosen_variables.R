...
p_final <- (row1 / row2 / row3 / row4) +
  ...
  plot_annotation(
    title = "Seasonal Overlap (DOY 133 - 286) of Environmental and Depth Metrics - Bluegreens",
    theme = theme(
...
ggsave(
  filename = "02_CYANOS/Figs/all_variables_visualized.png",
  plot = p_final,
...
# ── Read source CSVs ──
stats_waterlevel <- read.csv("02_CYANOS/CSVs/water_level.csv") %>%
  group_by(Year, Week) %>%
  slice(1) %>%
  ungroup() %>%
  select(Year, Week, WaterLevel_m)

stats_photic  <- read.csv("02_CYANOS/CSVs/final_photic_thermo.csv") %>%
  select(Year, Week, PZ, thermocline_depth)

stats_buoyancy <- read.csv("02_CYANOS/CSVs/final_buoyancy.csv") %>%
  select(-any_of("Date"))

stats_chem    <- read.csv("02_CYANOS/CSVs/final_chem.csv") %>%
  select(-any_of("Date"))

stats_metals  <- read.csv("02_CYANOS/CSVs/final_metals.csv") %>%
  select(-any_of("Date"))

stats_schmidt <- read.csv("02_CYANOS/CSVs/final_schmidt.csv") %>%
  select(-any_of("Date"))

stats_metdata <- read.csv("02_CYANOS/CSVs/final_metdata.csv") %>%
  rename(
    AirTemp_Avg   = weekly_airtempavg,
    WindSpeed_Avg = WindSpeed_Weekly_Average_m_s,
    Precip_Weekly = precip_weekly
...
stats_phytos  <- read.csv("02_CYANOS/CSVs/final_phytos.csv") %>%
...
write.csv(overall_summary, "02_CYANOS/CSVs/summary_statistics_overall.csv", row.names = FALSE)
...
write.csv(yearly_stats, "02_CYANOS/CSVs/yearly_variable_stats.csv", row.names = FALSE)
...
p_yearly <- (yrow1 / yrow2 / yrow3 / yrow4) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Yearly Median and Mean \u00b1 SD (DOY 133\u2013286) - Bluegreens",
    theme = theme(
      legend.position = "right",
      legend.text = element_text(size = 16),
      legend.title = element_text(size = 18),
...
ggsave(
  filename = "02_CYANOS/Figs/all_variables_yearly_stats.png",
  plot = p_yearly,
  width = 20, height = 14, units = "in", dpi = 1200, bg = "white"
)
