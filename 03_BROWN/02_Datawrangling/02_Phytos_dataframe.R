...
#adding columns with BrownAlgae_ugL max and the depth at which it occurs
phytos <- phytos_df %>% 
...
write.csv(phytos, "03_BROWN/CSVs/phytos.csv", row.names = FALSE)
phytos <- read.csv("03_BROWN/CSVs/phytos.csv")

####1. flora instrument data availability produces Figure S2####
# 1. Build plotting data
plot_dat <- phytos %>%
  filter(!is.na(BrownAlgae_ugL)) %>%
  mutate(
    Year = year(Date),
    DayOfYear = yday(Date)
  ) %>%
  select(Date, Year, Week, DayOfYear, BrownAlgae_ugL, Depth_m)

# 2. For each Year, get the row with the max concentration
max_totals_per_year <- plot_dat %>%
  group_by(Year) %>%
  slice(which.max(BrownAlgae_ugL)) %>%
  ungroup() %>%
  mutate(
    label_text = paste0(
      "Max: ",
      round(BrownAlgae_ugL, 1), " µg/L at ",
      Depth_m, " m"
    ),
...
phytosplot <- ggplot(plot_dat, aes(x = DayOfYear, y = as.factor(Year), group = Year)) +
  ...
  labs(
    x = "Day of Year",
    y = "Year",
    title = "Fluoroprobe Data Availability 2015-2024 - Brown Algae"
  ) +
  scale_x_continuous(
...
dir.create("03_BROWN/Figs/Data_availability", recursive = TRUE, showWarnings = FALSE)

ggsave(
  "03_BROWN/Figs/Data_availability/TotalPhytos2025pub.png",
  phytosplot,
  width = 8,
  height = 6,
  dpi = 400
...
if (!dir.exists("03_BROWN/Figs")) {
  dir.create("03_BROWN/Figs")
}

if (!dir.exists("03_BROWN/Figs/Phytos_viz")) {
  dir.create("03_BROWN/Figs/Phytos_viz")
}
...
for (type in types) {
  ...
  for (yr in years) {
    ...
    dir.create("03_BROWN/Figs/raw_flora_casts", recursive = TRUE, showWarnings = FALSE)
    
    # Save plot
    ggsave(
      filename = paste0("03_BROWN/Figs/raw_flora_casts/", type, "_", yr, "_raw_casts.png"),
      plot = plot_casts,
      width = 12,
      height = 10,
      dpi = 300
...
pigment_vars <- c("BrownAlgae_ugL") #if you want explore these "BrownAlgae_ugL", "Bluegreens_ugL", "BrownAlgae_ugL", "MixedAlgae_ugL"
...
DCM_metrics_depth2 <- DCM_metrics_depth1 %>%
  ...
  mutate(
    ...
    top5_mean = mean(BrownAlgae_ugL[Depth_m <= top5_depth], na.rm = TRUE),
    qualifies_DCM =
      BrownAlgae_ugL_DCM_depth > top5_depth &
      BrownAlgae_ugL_max_conc >= 1.5 * top5_mean
  ) %>%
  ungroup() %>%
...
pigment_vars <- c("BrownAlgae_ugL") #, "BrownAlgae_ugL", "Bluegreens_ugL", "BrownAlgae_ugL", "MixedAlgae_ugL"
...
for (var in pigment_vars) {
  ...
  for (yr in years) {
    ...
    ggsave(
      filename = paste0("03_BROWN/Figs/raw_flora_casts/", var, "_", yr, "_raw_casts.png"),
      plot = plot_casts,
      width = 12,
      height = 10,
      dpi = 300
...
final_DCM_metrics<- DCM_metrics_depth2|>
  mutate(max_conc = BrownAlgae_ugL_max_conc, 
         DCM_depth = BrownAlgae_ugL_DCM_depth)|>
...
depth_plot <- ggplot(boxplot_Data, aes(x = factor(Year), y = DCM_depth)) +
  ...
  ggtitle("A   Deep Chlorophyll Maximum (DCM) Depth - Brown Algae") +
...
mag_plot <- ggplot(boxplot_Data, aes(x = factor(Year), y = max_conc)) +
  ...
  ggtitle("B   Deep Chlorophyll Maximum (DCM) Peak Magnitude - Brown Algae") +
  scale_y_continuous(
...
write.csv(frame_weeks, "03_BROWN/CSVs/frame_weeks.csv", row.names = FALSE)
write.csv(final_phytos, "03_BROWN/CSVs/final_phytos.csv", row.names = FALSE)
...
sig_both <- p_depth / p_mag_nolegend +
  ...
  plot_annotation(
    title = "Year-wise Pairwise Significance (BH-adjusted) - Brown Algae",
    theme = theme(plot.title = element_text(face = "bold", hjust = 0.5))
  )

ggsave("03_BROWN/Figs/Phytos_viz/kruskal-wallis.png",
...
d <- ggplot(summary_df, aes(x = factor(Year))) +
  ...
  labs(
    y = "DCM depth (m)",
    x = "Year",
    title = "A   DCM Depth Median and Mean ± SE by Year - Brown Algae"
  ) +
...
p <- ggplot(summary_df, aes(x = factor(Year))) +
  ...
  labs(
    ...
    x = "Year",
    title = "B   DCM Magnitude Median and Mean ± SE by Year - Brown Algae"
  ) +
  theme_classic(base_size = 13) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggsave("03_BROWN/Figs/Phytos_viz/phytoplankton_summary.png", plot = combined_plot, width = 8, height = 10, dpi = 600)
